...
scp -i ~/.ssh/id_ed25519_hydro "$MONITORING_DIR/prometheus.yml" "hydro@$SERVER_IP:/tmp/prometheus.yml"
scp -i ~/.ssh/id_ed25519_hydro "$MONITORING_DIR/blackbox.yml" "hydro@$SERVER_IP:/tmp/blackbox.yml"

echo "Setting up directories and deploying monitoring stack..."
ssh -i ~/.ssh/id_ed25519_hydro "hydro@$SERVER_IP" << 'EOF'
    set -e
    echo "Creating /srv/monitoring directory..."
    ...
    mv /tmp/prometheus.yml /srv/monitoring/prometheus.yml
    mv /tmp/blackbox.yml /srv/monitoring/blackbox.yml
    chmod 644 /srv/monitoring/*.yml
